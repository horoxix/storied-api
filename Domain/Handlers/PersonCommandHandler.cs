...
public class PersonCommandHandler
{
    ...
    public async Task<Person> Handle(AddPersonCommand command)
    {
        LogHandlerEntry(nameof(AddPersonCommand));

        return await _personRepository.Add(_mapper.Map<Person>(command));
    }
    ...
    public async Task<Person> Handle(RecordBirthCommand command)
    {
        ...
        return await _personRepository.RecordBirth(_mapper.Map<Person>(command));
    }
